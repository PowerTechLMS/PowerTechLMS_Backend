...
public class EnrollmentsController : ControllerBase
{
    ...
    public async Task<ActionResult> Enroll([FromBody] EnrollRequest request)
    {
        try
        {
            return Ok(await _enrollmentService.EnrollAsync(UserId, request.CourseId));
    ...
    public async Task<ActionResult> AdminEnroll([FromBody] AdminEnrollRequest request)
    {
        try
        {
            return Ok(await _enrollmentService.AdminEnrollAsync(request, UserId));
        } catch(InvalidOperationException ex)
        {
            return BadRequest(new { message = ex.Message });
        }
    ...
    [HttpGet("my")]
    public async Task<ActionResult> GetMyEnrollments() => Ok(await _enrollmentService.GetUserEnrollmentsAsync(UserId));

    [HttpGet("course/{courseId}")]
    [Authorize(Policy = "EnrollmentView")]
    public async Task<ActionResult> GetCourseEnrollments(int courseId) => Ok(
        await _enrollmentService.GetCourseEnrollmentsAsync(courseId));

    [HttpGet("pending")]
    [Authorize(Policy = "EnrollmentView")]
    public async Task<ActionResult> GetPending() => Ok(
        await _enrollmentService.GetPendingEnrollmentsAsync(UserId, IsAdmin));
}
