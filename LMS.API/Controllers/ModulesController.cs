...
public class ModulesController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult> Create(int courseId, [FromBody] CreateModuleRequest request) => Ok(
        await _moduleService.CreateModuleAsync(courseId, request, UserId, IsAdmin));
    ...
    public async Task<ActionResult> Update(int courseId, int id, [FromBody] UpdateModuleRequest request)
    {
        try
        {
            return Ok(await _moduleService.UpdateModuleAsync(courseId, id, request, UserId, IsAdmin));
    ...
    public async Task<ActionResult> Delete(int courseId, int id)
    {
        try
        {
            await _moduleService.DeleteModuleAsync(courseId, id, UserId, IsAdmin);
            return NoContent();
        } catch(KeyNotFoundException)
        {
            return NotFound();
