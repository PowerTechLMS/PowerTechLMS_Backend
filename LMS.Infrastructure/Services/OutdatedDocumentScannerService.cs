    ...
    public async Task ScanAllDocumentsAsync()
    {
        ...
        foreach (var doc in documents)
        {
            try
            {
                ...
                {
                    _logger.LogInformation("[OutdatedScanner] Tài liệu {Id} chưa được xử lý AI hoặc không có nội dung text.", doc.Id);
                    continue;
                }
...
";
        ...
        try
        {
            ...
            if (startIndex >= 0 && endIndex > startIndex)
            {
                ...
                return System.Text.Json.JsonSerializer.Deserialize<OutdatedCheckResult>(jsonStr, new System.Text.Json.JsonSerializerOptions { PropertyNameCaseInsensitive = true }) 
    ...
    private async Task NotifyAllOutdatedDocumentsAsync(List<Document> documents)
    {
        ...
        sb.Append($"<p>Hệ thống AI đã phát hiện <strong>{documents.Count}</strong> tài liệu có dấu hiệu lỗi thời hoặc cần cập nhật mới:</p>");
        ...
        foreach (var doc in documents)
        {
            ...
            sb.Append($"<td style='border: 1px solid #dee2e6; padding: 12px; color: #d63384;'>{doc.OutdatedReason}</td>");
            sb.Append("</tr>");
        }
        sb.Append("</table>");

        sb.Append("<p style='margin-top: 25px;'>Vui lòng kiểm tra lại tính chính xác và cập nhật phiên bản mới nếu cần thiết.</p>");
        sb.Append($"<p><a href='{_config["FrontendUrl"]}/documents' style='display: inline-block; padding: 10px 20px; background-color: #0d6efd; color: white; text-decoration: none; border-radius: 5px;'>Quản lý tài liệu</a></p>");
        ...
        _logger.LogInformation("[OutdatedScanner] Đã gửi email tổng hợp cho {Count} tài liệu tới {UserCount} người dùng.", documents.Count, users.Count);
    }
