...
public class PythonEmbeddingService : IPythonEmbeddingService
{
    ...
    public async Task UpsertBatchAsync(string collectionName, List<VectorPoint> points)
    {
        ...
        var jsonContent = JsonSerializer.Serialize(
            points.Select(p => new { id = p.Id.ToString(), text = p.Text, payload = p.Payload }));
        ...
        try
        {
            var startInfo = new ProcessStartInfo
            {
                FileName = pythonPath,
                Arguments =
                    $"\"{_scriptPath}\" --input_json \"{tempFile}\" --model {_modelName} --qdrant_url \"{pythonQdrantUrl}\" --collection \"{collectionName}\"",
                RedirectStandardOutput = true,
                RedirectStandardError = true,
                UseShellExecute = false,
                CreateNoWindow = true,
