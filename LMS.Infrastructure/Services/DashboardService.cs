...
public class DashboardService : IDashboardService
{
    ...
    public async Task<LearnerDashboardResponse> GetLearnerDashboardAsync(int userId)
    {
        ...
        try
        {
            var userGroup = await _db.UserGroupMembers.Include(ugm => ugm.Group)
            ...
            var progresses = await _db.LessonProgresses.Include(lp => lp.Lesson)
                .Where(lp => lp.UserId == userId && !lp.IsDeleted).ToListAsync();

            var quizAttempts = await _db.QuizAttempts.Where(qa => qa.UserId == userId && !qa.IsDeleted).ToListAsync();

            // Weekly Mission Stats
            ...
            var activeDates = progresses.Where(p => p.CompletedAt.HasValue)
                ...
                .Union(quizAttempts.Select(q => q.SubmittedAt.Date))
            ...
            int totalPct = response.WeekMission.TotalTasks > 0 ? (response.WeekMission.DoneTasks * 100) / response.WeekMission.TotalTasks : 0;
            ...
            foreach (var e in enrollments) {
                ...
                var allLessons = e.Course.Modules?.SelectMany(m => m.Lessons ?? new List<LMS.Core.Entities.Lesson>()).OrderBy(l => l.SortOrder).ToList() ?? new();
                ...
                string? estTime = nextL != null ? $"{Math.Max(5, (nextL.VideoDurationSeconds + nextL.ReadingDurationSeconds) / 60)} phút" : "Hoàn tất";

                if (response.MyCourses.Count < 6) {
                    response.MyCourses.Add(new DashboardCourseDto(e.CourseId, e.Course.Title, e.IsMandatory ? "Bắt buộc" : "Gợi ý", 
                        $"{compCount}/{allLessons.Count}", "0/1", "#6366f1", "#a855f7", progPct, compCount, allLessons.Count, nextTitle, estTime));
                }

                if (compCount < allLessons.Count && response.TodayTasks.Count < 5) {
                    response.TodayTasks.Add(new DashboardTaskDto(e.Id, $"Bài: {nextTitle ?? e.Course.Title}", $"{compCount}/{allLessons.Count}", $"{progPct}%", false, false));
                }
            }

            // Practice
            var cIds = enrollments.Select(e => e.CourseId).ToList();
            var qzs = await _db.Quizzes.Where(q => cIds.Contains(q.CourseId) && !q.IsDeleted).Take(5).ToListAsync();
            foreach (var q in qzs) {
                var att = quizAttempts.FirstOrDefault(a => a.QuizId == q.Id);
                string stat = att == null ? "new" : (att.IsPassed ? "done" : "doing");
                if (response.TestPractice.Count < 4) response.TestPractice.Add(new DashboardQuizDto(q.Id, q.Title, $"{q.QuestionCount} câu hỏi", stat, stat == "new" ? "Chưa làm" : (stat == "done" ? "Hoàn thành" : "Đang làm"), "#6366f1"));
            }

            // Learning Profile Summary
            double entryScore = 3.0;
            double targetScore = 5.0;
            long totalSecs = progresses.Where(p => p.IsCompleted && p.Lesson != null).Sum(p => (long)p.Lesson!.VideoDurationSeconds + p.Lesson!.ReadingDurationSeconds);
            ...
            response.LearningProfile.PredictedScore = Math.Round(entryScore + ((double)totalPct / 100 * (targetScore - entryScore)), 1);
            ...
            var alerts = enrollments.Where(e => e.IsMandatory && e.Deadline.HasValue && e.Deadline.Value < now && e.Status != "Completed").Take(3).ToList();
            foreach (var a in alerts) response.HrMessages.Add(new HrMessageDto(a.Id, "HR", $"Quá hạn: {a.Course?.Title ?? "Khóa học"}", "Gấp", true));
