...
public class VectorDbService
{
    ...
    public VectorDbService(string qdrantUrl, ILogger<VectorDbService> logger, IPythonEmbeddingService embeddingService)
    {
    ...
    public async Task UpsertVectorAsync(Guid pointId, string content, IDictionary<string, object> metadata)
    {
        ...
        try
        {
            ...
            var point = new PointStruct { Id = pointId, Vectors = embedding, Payload = { ["content"] = content } };
        ...
        int limit = 5)
    {
        ...
        try
        {
            ...
            var results = await _client.SearchAsync(CollectionName, embedding, filter: filter, limit: (ulong)limit);
    ...
    private async Task<bool> EnsureCollectionReadyAsync()
    {
        ...
        try
        {
            ...
            } catch(Exception ex)
            {
                ...
                {
                    _logger.LogWarning(
                        "[VectorDbService] Note: Qdrant (VectorDB) is not available or connection failed: {Message}. AI-driven search features will be disabled. (Message suppressed for future retries)",
                        ex.Message);
                }
    ...
    private async Task EnsureCollectionExistsAsync()
    {
        ...
        if(!collections.Any(c => string.Equals(c, CollectionName, StringComparison.OrdinalIgnoreCase)))
        {
            _logger.LogInformation(
                "[VectorDb] Không tìm thấy collection '{CollectionName}'. Đang tạo mới...",
                CollectionName);
            try
            {
            ...
            {
                _logger.LogInformation(
                    "[VectorDb] Collection '{CollectionName}' đã được khởi tạo bởi tiến trình khác.",
                    CollectionName);
            }
        } else
        {
