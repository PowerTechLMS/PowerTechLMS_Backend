...
public class EssayService : IEssayService
{
    ...
    public async Task<EssayAttempt> StartAttemptAsync(int userId, int lessonId)
    {
        ...
        // Kiểm tra số lần làm bài trong khung giờ (nếu có cấu hình)
    ...
    public async Task<EssayResultResponse> SubmitAttemptAsync(int userId, int attemptId, SubmitEssayRequest request)
    {
        ...
        // Xóa câu trả lời cũ (nếu có từ bản nháp)
        ...
        // Chấm điểm bằng AI ngay lập tức
    ...
    private async Task<EssayResultResponse> ScoreAttemptAsync(int attemptId)
    {
        ...
        var questionsAndAnswers = attempt.Answers.Select(a => new
        {
        ...
        promptBuilder.AppendLine("Bạn là một chuyên gia chấm bài tự luận. Hãy dựa vào danh sách câu hỏi và câu trả lời dưới đây để chấm điểm.");
        ...
        promptBuilder.AppendLine("    { \"questionId\": 1, \"score\": 85, \"feedback\": \"Nhận xét chi tiết cho câu này (thang điểm 100)...\" }");
        promptBuilder.AppendLine("  ]");
        promptBuilder.AppendLine("}");
        promptBuilder.AppendLine("\nDANH SÁCH BÀI LÀM:");
        ...
        try
        {
            ...
            // Tìm khối ```json ... ``` hoặc ``` ... ```
            var match = System.Text.RegularExpressions.Regex.Match(jsonString, @"```json\s*(.*?)\s*```", System.Text.RegularExpressions.RegexOptions.Singleline);
            if (match.Success)
            {
            ...
            {
                match = System.Text.RegularExpressions.Regex.Match(jsonString, @"```\s*(.*?)\s*```", System.Text.RegularExpressions.RegexOptions.Singleline);
                if (match.Success)
                {
                    jsonString = match.Groups[1].Value;
                }
                ...
                {
                    // Nếu không có markdown block, thử lấy từ { đầu tiên đến } cuối cùng
            ...
            attempt.AiFeedback = root.TryGetProperty("aiFeedback", out var feedbackProp) ? feedbackProp.GetString() : "Không có nhận xét tổng quan.";
            ...
            if (root.TryGetProperty("answers", out var answerScores))
            {
                foreach (var ansScore in answerScores.EnumerateArray())
                {
                    ...
                    if (answer is not null)
                    {
                        ...
                        answer.AiFeedback = ansScore.TryGetProperty("feedback", out var f) ? f.GetString() : "";
                        
                        // Tính điểm cộng dồn dựa trên tỉ trọng thực tế của câu hỏi
            ...
            var attemptNumber = await _db.EssayAttempts
                .CountAsync(a => a.UserId == attempt.UserId && a.LessonId == attempt.LessonId && a.StartedAt <= attempt.StartedAt);
            ...
            return new EssayResultResponse(
                ...
                attempt.AiFeedback,
                attempt.Answers.Select(a => new EssayAnswerResultItem(
                    a.QuestionId,
                    a.Question.Content,
                    a.Content,
                    a.AiScore,
    ...
    public async Task<List<EssayAttemptSummary>> GetAttemptsByLessonAsync(int userId, int lessonId, bool isAdmin = false)
    {
        ...
        var count = 1;
        return attempts.Select(a => new EssayAttemptSummary(
            a.Id,
            count++,
            a.TotalScore,
            a.IsPassed,
    ...
    public async Task<EssayResultResponse> GetAttemptDetailAsync(int userId, int attemptId, bool isAdmin = false)
    {
        ...
        // Group by QuestionId và lấy câu trả lời có điểm (hoặc câu trả lời cuối cùng) để xử lý dữ liệu rác cũ
    ...
    public async Task<List<AdminEssayAttemptResponse>> GetAllAttemptsAsync()
    {
        ...
        return attempts.Select(a => new AdminEssayAttemptResponse(
            a.Id,
            a.UserId,
            a.User.FullName,
            a.LessonId,
            ...
            a.AiFeedback,
            a.CreatedAt,
            new List<EssayAnswerResultItem>() // Detailed answers loaded on demand
        )).ToList();
    }
    ...
    public async Task UpdateAttemptAsync(int attemptId, AdminUpdateEssayAttemptRequest request)
    {
        ...
        // Kiểm tra xem còn bất kỳ bài làm nào khác của bài học này đang Đạt hay không
        bool isAnyPassed = await _db.EssayAttempts
            .AnyAsync(a => a.UserId == attempt.UserId && a.LessonId == attempt.LessonId && a.IsPassed && a.Id != attempt.Id) || attempt.IsPassed;
...
LƯU Ý: Tổng ""weight"" của tất cả các câu hỏi phải luôn bằng 100. Trả lời thuần JSON, bằng tiếng Việt.";
        ...
        try
        {
            ...
            return JsonSerializer.Deserialize<List<EssayQuestionDto>>(jsonString, options) ?? new List<EssayQuestionDto>();
        }
        catch
        {
            return new List<EssayQuestionDto>();
    ...
    private async Task MarkLessonAsCompletedAsync(int userId, int lessonId, bool isCompleted)
    {
        ...
        {
            // Nếu bị đánh dấu rớt, kiểm tra và thu hồi chứng chỉ nếu có
            var certificate = await _db.Certificates
                .FirstOrDefaultAsync(c => c.UserId == userId && c.CourseId == lesson.Module.CourseId && c.Status == "Issued");

            if (certificate != null)
            {
                // Gọi Revoke qua CertificateService (adminId = 1 cho hệ thống)
                await _certificateService.RevokeCertificateAsync(certificate.Id, "Bài học trong khóa học bị hạ điểm dẫn đến không còn đủ điều kiện hoàn thành.", 1);
            }
        }
    }
}
