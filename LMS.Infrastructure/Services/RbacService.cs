...
public class RbacService : IRbacService
{
    ...
    public async Task<RoleDto> UpdateRolePermissionsAsync(int roleId, AssignPermissionsRequest request)
    {
        ...
        return new RoleDto(updated.Id, updated.Name, updated.Description, updated.IsSystem,
    ...
    public async Task DeleteRoleAsync(int roleId)
    {
        var role = await _db.Roles.FindAsync(roleId)
            ?? throw new KeyNotFoundException("Không tìm thấy Role.");
        if (role.IsSystem) throw new InvalidOperationException("Không thể xóa Role hệ thống.");
        role.IsDeleted = true;
    ...
    public async Task<UserRoleDto> UpdateUserRolesAsync(int userId, AssignRolesRequest request)
    {
        ...
        // Cập nhật legacy User.Role = role đầu tiên (tương thích ngược)
        var firstRole = await _db.Roles.FindAsync(request.RoleIds.FirstOrDefault());
        if (firstRole != null) user.Role = firstRole.Name;
        ...
        return new UserRoleDto(updated.Id, updated.FullName, updated.Email,
            updated.UserRoles.Select(ur => ur.Role.Name).ToList(), updated.Avatar);
    }
}
