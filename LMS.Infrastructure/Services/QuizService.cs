...
public class QuizService : IQuizService
{
        ...
        return await _db.Quizzes
            ...
            .Select(q => new QuizDetailResponse(
                ...
                _db.QuestionBanks
                    ...
                    .Select(qb => new QuestionBankResponse(
                        qb.Id,
                        qb.QuestionText,
                        qb.OptionA,
                        qb.OptionB,
    ...
    public async Task<Quiz> CreateQuizAsync(int courseId, CreateQuizRequest request)
    {
        ...
        var quiz = new Quiz
        {
            ...
            PassScore = 5, // Ép cứng pass 5 theo yêu cầu tư duy mới
    ...
    public async Task<QuestionResponse> AddQuestionAsync(int quizId, CreateQuestionRequest request)
    {
        ...
        return new QuestionResponse(
            question.Id, question.QuestionText, question.OptionA, question.OptionB,
            question.OptionC, question.OptionD, question.CorrectAnswer, (double)question.Points
        );
    }
    ...
    public async Task<QuizResultResponse> SubmitQuizAsync(int userId, int attemptId, SubmitQuizRequest request)
    {
        ...
        foreach (var submitted in request.Answers)
        {
            ...
            var isCorrect = ans.Question.CorrectAnswer.Equals(submitted.SelectedAnswer, StringComparison.OrdinalIgnoreCase);
        ...
        await _db.SaveChangesAsync(); // Lưu để ProgressService nhìn thấy kết quả thi mới nhất

        // TỰ ĐỘNG KIỂM TRA VÀ CẤP CHỨNG CHỈ
        if (attempt.IsPassed)
        {
            ...
            if (progress.IsCompleted)
            {
                ...
                // Cập nhật trạng thái Enrollment để Leaderboard nhận diện
    ...
    public async Task<StartQuizResponse> StartQuizAsync(int userId, int quizId)
    {
        ...
        var attempt = new QuizAttempt { UserId = userId, QuizId = quizId, Status = "InProgress", StartedAt = DateTime.UtcNow, RemainingSeconds = quiz.TimeLimitMinutes * 60 };
        _db.QuizAttempts.Add(attempt);
        await _db.SaveChangesAsync();

        foreach (var q in questions) _db.QuizAnswers.Add(new QuizAnswer { AttemptId = attempt.Id, QuestionId = q.Id });
        await _db.SaveChangesAsync();
    ...
    public async Task SaveAnswerDraftAsync(int attemptId, int userId, int questionId, string? selected)
    {
        ...
        if (ans != null) { ans.SelectedAnswer = selected; ans.AnsweredAt = DateTime.UtcNow; await _db.SaveChangesAsync(); }
    }
    ...
    public async Task UpdateRemainingTimeAsync(int attemptId, int userId, int remainingSeconds)
    {
        ...
        if (attempt != null) { attempt.RemainingSeconds = remainingSeconds; await _db.SaveChangesAsync(); }
    }
